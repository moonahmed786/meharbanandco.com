...
class OhioImporter {
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( $this->plugin_page === $hook ) {
			...
			wp_enqueue_script( 'ocdi-main-js', PT_OCDI_URL . 'assets/js/min/main.min.js' , array( 'jquery', 'jquery-ui-dialog' ), PT_OCDI_VERSION );
	...
	private function final_response() {
		...
		//if ( empty( $this->frontend_error_messages ) ) {
			...
			$response['message'] .= sprintf(
				__( '%1$s%3$sThat\'s it, all done! The demo import has finished. Please check the demo and make sure that everything has imported correctly.', 'ohio-importer' ),
				'<div class="notice o-notice success"><div class="holder"><i class="bi bi-check2-circle"></i>',
				'<br>',
				'<p>',
				'</p>',
				'</div></div>'
		...
		/*}
			...
			$response['message'] .= sprintf(
				...
				'<div class="notice o-notice notice-warning"><p>',
				'<br>',
				'<strong>',
				'</strong>',
